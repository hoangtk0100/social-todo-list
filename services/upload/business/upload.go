...
func (biz *business) Upload(ctx context.Context, data []byte, folder, fileName string, contentType string) (*core.Image, error) {
...
func (biz *business) UploadLocal(ctx context.Context, fileHeader *multipart.FileHeader, data []byte, folder, fileName string) (*core.Image, error) {
	width, height, err := util.GetImageDimension(data)
	if err != nil {
		return nil, core.ErrBadRequest.WithError(entity.ErrFileNotImage.Error()).WithDebug(err.Error())
	}
